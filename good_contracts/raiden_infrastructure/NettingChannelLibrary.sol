...
library NettingChannelLibrary {
     ...
     struct Participant {
           ...
           //Total amount of token transferred to this smart contract trough the
           //'deposit' function, note that direct token transfer cannot be tracked
           // and will be burned
           uint256 balance;
           ...
           //The latest known transferred_amount from this node to the other participant used to compute the net balance of settlement
           uint256 transferred_amount;

           //value used to order transfers and only accept the latest on calls to update
           //, this will only be relevant after with #182 or #293 are implemented
           uint64 nonce;
     ...
     // Deposit an amouunt to the channel. At least one of the participants must deposit
     //before the channel is opened.
     //param 'amount' is the amount to be deposited to the address
     //returns Success if the transfer was successful
     // returns the new balacne of the invoker
          ...
          public
    {
         ...
         // and only when this function is called, i.e. this value cannot be updated afterwards

         //An empty value means that the closer never received a transfer, or he is intentionally
         //not providing the latest transfer, in which case the closing party is going to lose the tokens
         //that were transferred to him.
         if (signature.length == 65) {
              ...
              //update the structure of the counterparty with its data provided by the
              //closing node
    ...
    //updates the counterparty transfer after closing
          ...
          public
   {
         ...
         //Only a participant can call updateTransfer
         ...
         transfer _address = recoverAddressFromSignature(nonce, transferred_amount, locksroot, extra_hash, signature);
         require(transfer_address == self.closing_address);

         //Update the structure of the closer with its data provided by the counterparty
   ...
   //this is the method that they use to recover the address from a signature using ECRECOVER
   //you split the signature and then put the signed_hash and the three horcruxes into ecrecover and return that
   //apparently based on this construction, it should be an address
        ...
        returns (address)
  {
       ...
       signed_hash = keccak256(nonce, transferred_amount, locksroot, this, extra_hash);
  ...
  //lock_encoed is the lock
  //merkle_proof the merkle proof
  //secret is the secret
  function withdraw(
    Data storage self,
    bytes32 locked_encoded,
    ...
    public
  {
       ...
       counterparty.withdrawn_locks[hashslock] = true;

       require (expiration > = block.number);
       require(counterparty.locksroot == h);

       //This implementation allows for each transfer to be set only once, so it's safe to
       //update the transferred_amount in place.

       //Once third parties are allowed to update the counter party transfer
       //the locksroot might change, if the locksroot changes the transfered_amount must also be reset and locks must be re-withdrawn, so this
       //is also safe.

       //This may be problematic if an update changes the transferred_amount
       // but not the locksroot since the locks don't need to be withdraw, the differenace in the
       //transferred_amount must be accounted for

       counterparty.transferred_amount += amount;
     }

     //settles the balance between the two parties
     //settles the balances of the two parties for the channel
     //the participants with netted balances
         ...
         public
    {
         ...
         counter_net =(counter_party.balance + closing_party.transferred_amount - counter_party.transferred_amount);

         // Direct token transfers done through the token transfer function cannot be accounted for,
         //these superfluous tokens will be burned, this is because there is no way to tell which participant (if any)
         ...
         //When the counter party does not porvide the closing party transfer,
         // then the 'counter_net' may be larger than the 'total_deposit', without
         // the min the token transfer fail and the token is locked
         counter_amount = min(counter_net, total_deposit);

         //When the counter party does not provide the closing party tranfser,
         //then 'counter_amount' may be negative and the transfer fails, force the value to 0
         counter_amount = max(counter_amount, 0);

         //At this point 'counter_amount' is bteween [0,total_deposit], so this
         //is safe
    ...
    {
        uint8 n;
        //Return index of participants, or throw
...
// - http://solidity.readthedocs.io/en/develop/assembly.html
          ...
          returns (bytes32 r, bytes32 s, uint8 v)
     {
         ...
         assembly {
             ...
             //
             // 'byte' is not working due to Solidity parser so they use the next best option
             //'and'
    ...
    function min(uint a, uint b) pure internal returns (uint) {return a > b ? b : a;}
    function max(uint a, uint b) pure internal returns (uint) {return a > b ? a : b;}
}
