...
//This sets up some of the math necessary for elliptic curve cryptography
//Primarily draws from the code provided by Andreas Oloffson and the descriptions
// http://www.secg.org/sec1-v2.pdf
// http://www.secg.org/sec2-v2.pdf

//Secp256k1
//Standards for efficient cryptography:
//The parameters are over F_p
//the length in bits of the field size in 256
//k means that we're using a koblitz curve instead of verifiably random parameters
library Secp256k1 {
   //TODO separate curve for crypto primitives
   // Field size
    ...
    // For later (WHY ARE THESE COMMENTED OUT??)
    ...
    function validateSignature(bytes32 message, uint[2] rs, uint[2] Q) internal constant returns (bool) {
         ...
         Px = mulmod(P[0], mulmod(Px, Py, p), p);
         return Px % n == rs[0]; //returns a boolean, only true if it is the correct signature
    }

    // see Curve.compress
    function compress(uint[2] P) internal constant returns (uint8 yBit, uintx) {
         x = P[0];
         yBit = P[1] == 1 ? 1 : 0;
    }

    // see Curve.decompress
    ...
    function _addMixed(uint[3] memory P, uint[2] memory Q) internal constant (uint[3] memory R) {
         if (P[2] == 0) {
              return [Q[0], Q[1], 1];
         }
         if (Q[1] == 0) {
    ...
    // same as addMixed but the params are different and this actually mutates P.
    ...
    //Point doubling 2*P
    // the params are Px, Py, Pz stored in uint[3] memory P
    ...
    function _mul(uint d, uint[2] memory P) internal constant returns(uint[3] memory Q) {
         uint p = pp;
         if (d == 0) {
              return; //"TO-DO" on github document
         }
         ...
         assembly
         {
              ...
              loop:
                  ...
                  dm := mod(d, 32)
                  mstore8(add(dwPtr, i), dm) // Don't store as signed - convert when reading the SEC documentation
         ...
         INV[9] = mulmod(PREC[2][2], INV[8], p); //zInv
         for (k = 0; k < 7; k++) {
